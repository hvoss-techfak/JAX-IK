...
def process_targets(script_name, num_batches):
    ...
    """
    successful_targets = []
    targets = [(np.asarray([0.15, 0, 0.35]) + np.random.uniform(-0.5, 0.5, (3))).tolist() for _ in range(BATCH_SIZE)]
...
def main():
    ...
    implementations = [
        ...
        ["tensorflow non c cpu", "ik_tensorflow_non_c.py", ["--target_points", targets_str, "--subpoints", "0", "--cpu_only"]],
        ["tensorflow non function cpu", "ik_tensorflow_non_function.py", ["--target_points", targets_str, "--subpoints", "0", "--cpu_only"]],
        ["tensorflow non c non function cpu", "ik_tensorflow_non_c_function.py", ["--target_points", targets_str, "--subpoints", "0", "--cpu_only"]],
    ]
