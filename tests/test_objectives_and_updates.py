...
def solver_with_fake_sdf(solver, fake_sdf):
    ...
    if fk_solver.mesh_data is None:
        ...
        verts = np.array([
            [0,0,0,1], [0.01,0,0,1], [0,0.01,0,1], [0,0,0.01,1],
            [0.01,0.01,0,1], [0.01,0,0.01,1], [0,0.01,0.01,1], [0.01,0.01,0.01,1]
        ], dtype=np.float32)
        fk_solver.mesh_data = {"vertices": jnp.asarray(verts), "faces": np.array([[0,1,2]])}
    return solver
...
def _run_solver_once(solver, mandatory=None, optional=None, traj=False):
    ...
    if not mandatory:
        ...
        mandatory = [DistanceObjTraj(bone_name="left_wrist", target_points=[[0.01,0.01,0.01]], use_head=True, weight=0.05)]
...
def test_distance_obj_and_update(solver):
    t1 = [0.0,0.0,0.3]
    obj = DistanceObjTraj(bone_name="left_index3", target_points=[t1], use_head=True, weight=1.0)
    ...
    assert loss2 < 1e-3, "DistanceObjTraj did not converge sufficiently to second target"


def test_bone_direction_obj(solver):
    obj = BoneDirectionObjective(bone_name="left_elbow", directions=[[0,1,0]], weight=0.5)
    _run_solver_once(solver, mandatory=[obj])
    p = obj.get_params(); p["weight"] = 1.0; obj.update_params(p)
    _run_solver_once(solver, mandatory=[obj])
...
def test_equal_distance_obj(solver):
    ...
    p = obj.get_params(); p["weight"] = 0.4; obj.update_params(p)
    _run_solver_once(solver, optional=[obj], traj=True)
...
def test_derivative_obj_orders(solver):
    for order in (1,2,3):
        ...
        _run_solver_once(solver, optional=[obj], traj=True)
        p = obj.get_params(); p_weight = p["weight"]; p["weight"] = p_weight * 2; obj.update_params(p)
        _run_solver_once(solver, optional=[obj], traj=True)
...
def test_combined_derivative_obj(solver):
    ...
    _run_solver_once(solver, optional=[obj], traj=True)
    p = obj.get_params(); p["weights"] = [0.02,0.02,0.02]; obj.update_params(p)
    _run_solver_once(solver, optional=[obj], traj=True)


def test_bone_relative_look_obj(solver):
    mods = [(0, 0.01), (1, -0.01)]
    obj = BoneRelativeLookObj(bone_name="left_elbow", use_head=True, modifications=mods, weight=0.5)
    _run_solver_once(solver, optional=[obj])
    p = obj.get_params(); p["modifications"] = [(0,0.02)]; obj.update_params(p)
    _run_solver_once(solver, optional=[obj])


def test_sphere_collision_penalty(solver):
    collider = {"center": [0.0,0.0,0.0], "radius": 0.001}
    obj = SphereCollisionPenaltyObjTraj(sphere_collider=collider, weight=0.2, min_clearance=0.0)
    _run_solver_once(solver, optional=[obj], traj=True)
    p = obj.get_params(); p["weight"] = 0.4; obj.update_params(p)
    _run_solver_once(solver, optional=[obj], traj=True)
...
def test_sdf_collision_penalty(fake_sdf, solver):
    ...
    obj = SDFCollisionPenaltyObj(bone_name="left_wrist", sdf=fake_sdf, num_samples=3, weight=0.2)
    _run_solver_once(solver, optional=[obj])
    params = obj.get_params(); params["weight"] = 0.4; obj.update_params(params)
    _run_solver_once(solver, optional=[obj])


def test_sdf_self_collision_penalty(solver_with_fake_sdf):
    solver = solver_with_fake_sdf
    obj = SDFSelfCollisionPenaltyObj(bone_names=solver.controlled_bones, num_samples_per_bone=2, min_dist=0.0, weight=0.5)
    _run_solver_once(solver, optional=[obj])
    p = obj.get_params(); p["weight"] = 1.0; obj.update_params(p)
    _run_solver_once(solver, optional=[obj])
...
def test_distance_obj_fk_accuracy_after_update(solver, model_path):
    ...
    obj = DistanceObjTraj(bone_name=end_bone, target_points=[old_target], use_head=False, weight=1.0)
